...
class TorchTests(unittest.TestCase):
    ...
    #     hvd.init()
    #     tensors = [torch.FloatTensor(
    #         10) if i % 2 else torch.cuda.FloatTensor(10) for i in range(5)]
    #     try:
    ...
    # model parallel, do not fuzz it for now
    ...
    ################################################################################
    ################# below are unittests related to alltoall ######################
    ################################################################################
    
    # horovod alltoall is less frequently used, skip their fuzz for now
    ...
    #     if hvd.ccl_built():
    #         self.skipTest(
    #             "Multiple process sets currently do not support CCL.")
    ...
    #         tensor = self.cast_and_place(tensor, dtype)
    #         collected, received_splits = hvd.alltoall(
    #             tensor, splits, process_set=this_set)
    #         tensor, collected = self.convert_cpu_fp16_to_fp32(
    #             tensor, collected)

    #         assert collected.data.min() == rank, 'hvd.alltoall produces incorrect collected tensor'
    #         assert collected.data.max() == rank, 'hvd.alltoall produces incorrect collected tensor'
    #         assert collected.numel() == sum(rk + 1 for rk in set_ranks) * 2**(dim -
    #                                                                           1), 'hvd.alltoall collected wrong number of values'
    ...
    ################################################################################
    ################# below are unittests related to allgather #####################
    ################################################################################
    
    # since distributed optimizer is equivlant to allgather, it is less used
    # we do not fuzz these method for now, see the GitHub issue for more
    # https://github.com/horovod/horovod/issues/2027#issuecomment-644412798
    ...
    ################################################################################
    ################## below are unittests related to process set ##################
    ################################################################################

    # process set is an advanced feature in horovod, we skip it in our fuzzer for now
    #https://horovod.readthedocs.io/en/stable/process_set_include.html
    ...
    #     for dtype, dim in itertools.product(dtypes, dims):
    #         torch.manual_seed(1234)
    #         even_rank_tensor = torch.FloatTensor(
    #             *([17] * dim)).random_(-100, 100)
    #         odd_rank_tensor = torch.FloatTensor(
    #             *([17] * dim)).random_(-100, 100)
    #         if rank in even_ranks:
    #             tensor = self.cast_and_place(even_rank_tensor, dtype)
    #             summed = hvd.allreduce(
    #                 tensor, average=False, process_set=even_set)
    #         elif rank in odd_ranks:
    #             tensor = self.cast_and_place(odd_rank_tensor, dtype)
    #             summed = hvd.allreduce(
    #                 tensor, average=False, process_set=odd_set)
    #         tensor, summed = self.convert_cpu_fp16_to_fp32(tensor, summed)
    #         if rank in even_ranks:
    ...
    #     for dtype, dim in itertools.product(dtypes, dims):
    #         torch.manual_seed(1234)
    #         even_rank_tensors = [torch.FloatTensor(
    #             *([17] * dim)).random_(-100, 100) for _ in range(5)]
    #         odd_rank_tensors = [torch.FloatTensor(
    #             *([17] * dim)).random_(-100, 100) for _ in range(5)]
    #         if rank in even_ranks:
    #             tensors = [self.cast_and_place(
    #                 tensor, dtype) for tensor in even_rank_tensors]
    #             summed = hvd.grouped_allreduce(
    #                 tensors, average=False, process_set=even_set)
    #         elif rank in odd_ranks:
    #             tensors = [self.cast_and_place(
    #                 tensor, dtype) for tensor in odd_rank_tensors]
    #             summed = hvd.grouped_allreduce(
    #                 tensors, average=False, process_set=odd_set)
    #         tensors, summed = zip(
    #             *[self.convert_cpu_fp16_to_fp32(t, s) for t, s in zip(tensors, summed)])
    #         if rank in even_ranks:
    ...
    #             tensor.requires_grad_()
    #         summed = hvd.grouped_allreduce(
    #             tensors, average=False, process_set=this_set)
    ...
    #         root_tensor = self.cast_and_place(root_tensor, dtype)
    #         broadcasted_tensor = hvd.broadcast(
    #             tensor, root_rank, process_set=this_set)
    #         tensor, root_tensor, broadcasted_tensor = \
    #             self.convert_cpu_fp16_to_fp32(
    #                 tensor, root_tensor, broadcasted_tensor)
    #         if rank != root_rank:
    ...
    #         obj, root_rank=set_ranks[0], process_set=this_set)
    #     self.assertDictEqual(obj, expected_obj)
    ...
    #     tensor_sizes = [17, 32, 81, 12, 15, 23, 22] * 5
    #     tensors = [random_sparse_tensor(d0, 10) for d0 in tensor_sizes]
    #     allreduced_tensors = [hvd.allreduce(
    #         t.to_dense(), process_set=this_set) for t in tensors]
    ...
    #             assert torch.allclose(v.flatten(), all_v.flatten()[
    #                                   start:start+v.numel()])
    #     else:
    #         for start in range(0, all_v.numel(), v.numel()):
    ...
    #             assert not torch.allclose(v.flatten(), all_v.flatten()[
    #                                       start:start + v.numel()])
    ...
    #         even_barrier_time = (even_barrier_time_end -
    #                              even_barrier_time_start).total_seconds()
    #         self.assertTrue(even_barrier_time >= 5)
    #     # No stall time for odd ranks
    #     elif hvd.rank() in odd_ranks:
    #         hvd.barrier(odd_set)
    #         odd_barrier_time_end = datetime.now()
    #         odd_barrier_time = (odd_barrier_time_end -
    #                             odd_barrier_time_start).total_seconds()
    ...
    ################################################################################
    ######## below are unittests that cannot be fuzzed: no arguments needed ########
    ################################################################################
    ...
    #             [1, 2, 3], dtype=torch.float32), name='test_allreduce').numpy()
